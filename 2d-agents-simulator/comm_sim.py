...
class NeuralAttacker(Attacker):
    ...
    def learn(self):
        ...
        #self.delta_y, self.delta_x = idx2coords[idx]
        #self.delta_y = round(out_data[0] * 2)
        #self.delta_x = round(out_data[1] * 2)
    ...
    def feedInput(self, state):
        ...
        #poi_x -= att_x
        #poi_y -= att_y
        #self.delta_x = norm(self.delta_x)
        #self.delta_y = norm(self.delta_y)
        #self.delta_y = norm2(self.delta_y)
        #self.delta_x = norm2(self.delta_x)
        #self.action = self.net.activate([self.delta_y, self.delta_x])
    ...
    for genome in genomes:
        ...
        for i in range(len(end_positions)):
            ...
            while not e.game_ended():
                ...
                steps+=1
                t = 0
                att_y, att_x = e.getLocation(ID_ATTACKER)
                #print("ATTACKER POS: {} {}".format(att_y, att_x))
                def norm(x):
                    if x < 0:
                        return 2
                    elif x > 0:
                        ...
                        return 0
                norm_y, norm_x = norm(y-att_y), norm(x - att_x)
                #norm_y, norm_x = (y - att_y) / MAP_DIM, (x - att_x) / MAP_DIM
                #print("SENDING: {} {}".format(norm_y, norm_x))
                out_receiver = [-1, -1]
                while t < T_MAX:
                    out_receiver[0] = norm_y
                    out_receiver[1] = norm_x
                    out_sender = net_send.advance(out_receiver, dt, dt)
                    attacker.listen(out_sender, dt, dt)
                    t+=1

                #attacker.learn()
                #if attacker.delta_x == norm_x or attacker.delta_y == norm_y:
                    #bonus -= 1
                #print("RECEIVED: {} {}".format(attacker.delta_x, attacker.delta_y))
                e.executeTimestep()
            #print("END GAME")

            fitness = -(manhattan_distance((x,y), e.getLocation(ID_ATTACKER)))
...
def viz(send_genome, rec_genome, send_config, rec_config):
    ...
    for y,x in pos:
        ...
        out_receiver = [1,1,1]
        while t < T_MAX:
            out_receiver[0] = feedback
            out_receiver[1] = norm_y
            out_receiver[2] = norm_x
            out_sender = net_send.advance(out_receiver, dt, dt)
            out_receiver = attacker.listen(out_sender, dt, dt)
            outs.append(out_sender)
            attacker.learn() 
            f_temp = (-1 * ((attacker.poi_x - x)**2 + (attacker.poi_y - y)**2))
            if f_temp == 0:
                feedback = 1
            else:
                feedback = 0
