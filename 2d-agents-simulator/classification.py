...
class NeuralAttacker(Attacker):
    ...
    def learn(self):
        ...
        self.poi_y = out_data[0]
        self.poi_x = out_data[1]
...
def eval_genomes(genomes, send_config, rec_config):
    ...
    for genome in genomes:
        ...
        for y,x in pos:
            ...
            while t < T_MAX:
                out_sender = net_send.advance([y, x], dt, dt)
                outs = attacker.listen(out_sender, dt, dt)
                attacker.learn()
                fitness += (-1 * ((attacker.poi_x - x)**2 + (attacker.poi_y - y)**2))
                t+=1
...
def viz(send_genome, rec_genome, send_config, rec_config):
    ...
    pos = [(0, 0), (0, 1), (1, 0), (1, 1), (0, 2), (1, 2), (2, 0), (2,1), (2,2)]
    for y, x in pos:
        ...
        while t < T_MAX:
            out_sender = net_send.advance([y, x], dt, dt)
            output = attacker.listen(out_sender, dt, dt)
        ...
        if attacker.poi_y == y and attacker.poi_x == x:
            cont+=1
        fopen = open("signals", 'a+')
        fouts = [i[0] for i in outs]
        fopen.write(' '.join(list(map(str, fouts))))
        fopen.write('\n')
        fopen.close()
