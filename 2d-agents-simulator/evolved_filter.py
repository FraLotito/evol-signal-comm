...
decompose = {}
decompose[(2, 1)] = [(2, 0), (0, 1)]
decompose[(2, 0)] = [(2, 0)]
decompose[(2, 2)] = [(2, 0), (0, 2)]
decompose[(1, 2)] = [(1,0), (0,2)]
decompose[(1, 1)] = [(1,0), (0,1)]
decompose[(1, 0)] = [(1,0)]
decompose[(0, 1)] = [(0,1)]
decompose[(0, 2)] = [(0,2)]
decompose[(0, 0)] = [(0,0)]
...
class NeuralAttacker(Attacker):
    def __init__(self, id, genome, config):
        ...
        #self.net_rec = neat.nn.FeedForwardNetwork.create(genome, config)
        self.action = []
    ...
    def listen(self, inputs, T_MAX, dt):
        ...
        #self.out = self.net_rec.activate(inputs)
        return self.out
    ...
    def learn(self):
        ...
        #self.poi_y, self.poi_x = associazioni[idx]
        #self.move = round(out_data*5)
...
def eval_genomes(genomes, send_config, rec_config):
    ...
    for genome in genomes:
        ...
        fitnesses = []
        pos = [(2, 0), (0, 2), (1, 0), (0, 1), (2, 2), (1, 1), (2, 1), (1, 2)]
        pairs = set()
        for i in pos:
            y, x = i[0], i[1]
            ...
            pred = (attacker.poi_y, attacker.poi_x)
            #pred = attacker.move
            if pred not in decompose[(y, x)]:
                f -= 1

            fitnesses.append(f)
        
        fit = np.mean(fitnesses)

        if not isinstance(send.fitness, list):
            send.fitness = []
            send.fitness.append(fit)
        else:
            send.fitness.append(fit)
        if not isinstance(rec.fitness, list):
            rec.fitness = []
            rec.fitness.append(fit)
        else:
            rec.fitness.append(fit)
