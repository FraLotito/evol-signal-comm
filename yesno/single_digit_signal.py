...
class NeuralAttacker():
    ...
    def listen(self, inputs, dt, target):
        oggetti = [[0, 1, 1], [1, 0, 1], [1, 1, 0], [0, 1, 0], [0, 0, 0]]
        #oggetti = [[0, 1], [1, 0], [1, 1], [0,0]]
        inputs.extend(oggetti[target])
        #inputs.append(target)
        self.out = self.net_rec.advance(inputs, dt, dt)
    
    def learn(self):
        #self.poi_y = round(self.out[0]*SUP_LIM)
        #self.poi_y = round(self.out[0])
        #self.poi_x = int(self.out[1]*MAP_DIM)
        #print("PRED: {} {}".format(self.poi_y, self.poi_x))
        
        self.poi_y = np.argmax(self.out)
        #self.poi_y = random.randint(0,1)
...
def eval_genomes(genomes, send_config, rec_config):
    ...
    for genome in genomes:
        ...
        for y in range(len(oggetti)):
            for target in range(len(oggetti)):
                dt = 1
                t = 0
                T_MAX = 10
                #norm_y = y / SUP_LIM
                ...
                while t < T_MAX:
                    ...
                    #out1[0] += random.gauss(mean_error, 0.2)
                    #out1[0] = int(out1[0])
                    receiver.listen(out1, dt, target)
                
                receiver.learn()

                if receiver.poi_y == 0 and y == target:
                    fitnesses.append(-3)
                elif receiver.poi_y == 1 and y != target:
                    fitnesses.append(-1)
...
def plot(send_genome, rec_genome, send_config, rec_config):
    ...
    for y in range(len(oggetti)):
        for target in range(len(oggetti)):
            ...
            #y = random.randint(0, len(oggetti) - 1)
            ...
            if receiver.poi_y == 1 and y == target:
                cont+=1
            elif receiver.poi_y == 0 and y != target:
                cont += 1
    print("Result: {} / {}".format(cont, len(oggetti)**2))
    """
...
winner_send, winner_rec = run(population_send, population_rec, n=1000)
