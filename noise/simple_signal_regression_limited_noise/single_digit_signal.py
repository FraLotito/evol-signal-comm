...
def eval_genomes(genomes, send_config, rec_config):
    ...
    for genome in genomes:
        ...
        for Y in range(MAP_DIM):
            ...
            #fitness -= abs(y - receiver.poi_y)

            #fitnesses.append(fitness)
            #print(fitnesses)
        #print((id_send, id_rcv), sum(fitnesses))
...
def run(population_send, population_rec, fitness_function=eval_genomes, n=None):
    ...
    while n is None or k < n:
        ...
        #print("BEST POPULATION: {}, {}, id: {}".format(best_send.fitness, best_rec.fitness, (best_send.key, best_rec.key)))
            ...
            #population_rec.best_genome = best_rec
        if ZOOM:
            plot(population_send.best_genome, population_rec.best_genome, population_send.config, population_rec.config)

        if SAVE_NETWORKS:
            fopen = open("fitness", "a+")
            fopen.write(str(population_send.best_genome.fitness))
            fopen.write('\n')
            fopen.close()
            pickle_out = open("sender_{}.pickle".format(k),"wb")
            pickle.dump(population_send.best_genome, pickle_out)
            pickle_out.close()
...
def viz(send_genome, rec_genome, send_config, rec_config):
    ...
    if cont > last_signal_result:
        last_signal_result = cont
        ...
        plt.legend()
        plt.savefig('{}_signal.pdf'.format(cont))
...
def plot(send_genome, rec_genome, send_config, rec_config):
    ...
    if cont > last_signal_result:
        ...
        plt.ylabel('amplitude')
        plt.legend()
        plt.savefig('{}_signal.jpeg'.format(cont), dpi=600)
